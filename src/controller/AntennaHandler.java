...
/**
 * This AntennaHandler class is a runnable class that listens for Cards to come
 * from the server and sends them to the apporiate Hands or CardId events
 * 
...
public class AntennaHandler extends Handler {
	...
	/**
	 * Condition variable to ensure that at least one card request gets sent
	 * between each antenna change.
	 */
	...
	/**
	 * Request a card from the server. Return when something other than NOCARD
	 * is received.
	 * 
	 ...
	 *             if the thread waiting between card requests is interrupted
	 */
	private String waitForMessage() throws IOException, InterruptedException {
		// System.out.println("Card request being sent from thread " +
		...
		// System.out.println("Received " + message.substring(0,
		// MESSAGE_LENGTH));
	 ...
	 * 
	 * @param str
	 *            - String that contains the ID number and the location found
	 */
	private void process(final String str) {
		new Thread("Card processor") {
			public void run() {
				...
				// } else {
				int position = Integer.parseInt(str.substring(POSITION,
						MESSAGE_LENGTH)) - 1;

				// remember the last card the blind person scanned. otherwise
				// lay cards normally
				// if(blindDirection.ordinal() == position){

				// System.out.println("blind player at " +
				// blindDirection.ordinal() + " scanned card " + thisCard);
	 ...
	 *             command to happen
	 */
	public void switchHand(String handID) throws IOException,
		...
		synchronized (out) {
			...
			// If a card hasn't been requested since the last antenna switch,
			// wait
			while (!cardRequestSent) {
				...
				// Thread.currentThread().getName());
				out.wait();
				// System.out.println("Awakened thread " +
				// Thread.currentThread().getName());
			}
	 ...
	 * @throws InterruptedException
	 */
	public void switchHand(final Direction turn) throws IOException,
			InterruptedException {
	 ...
	 *             the connection failed
	 * @throws InterruptedException
	 *             the cycling thread was interrupted
	 */
