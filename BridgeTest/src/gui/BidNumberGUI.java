...
/**
 * The panel that allows the user to set the number of tricks to win for
 * the contract.
 * 
 */
public class BidNumberGUI extends JPanel implements ActionListener {
	...
	private JPanel createButtonPanel(){
		...
		for (int i = 0; i < buttons.length; i++){
			...
			mainPanel.add(GUIUtilities.packageButton(buttons[i], FlowLayout.CENTER));
			
			// for testing, only 4 bids is allowed
			buttons[i].setEnabled(false);
			
		}
		
		// only bid 4 is allowed
